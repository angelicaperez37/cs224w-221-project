...
class LVQ():
	...
	def __init__(self):
		...
		for j in range(self.numCentroids):
			centroid = defaultdict(int)
			self.clusters[j] = []
			for key in f:
				if f[key] > 1:
					centroid[key] = (float(random.randint(0,20)))
				else:
					centroid[key] = random.random()
	...
	# returns total gradient loss of two vecs
	def computeLVQGradientLoss(self, features, weights, label):
		scalar =  2 * self.computeScore(features, weights) - label
		return abs(scalar)
	...
	def computeDistance(self,features, centroid):
		dist = 0.0
		for i,key in enumerate(features.keys()):
			dist += (features[key]-centroid[i])**2
		...
		for i in xrange(num_iter):
			...
			for game in allGames:
				...
				for i in self.centroids:
					...
					distList.append((i, centroid, self.computeLVQGradientLoss(features,centroid, int(didWin))))
				minIdx, minCentroid, minDist = min(distList ,key=lambda item:item[2])
				print minIdx
				...
				if minIdx not in newClusters:
					newClusters[minIdx] = [teamName+"_"+matchID]
				else:
					newClusters[minIdx].append(teamName+"_"+matchID)
				for key in newClusters:
					print "key: ", str(key)
					print len(newClusters[key])
